...
| 自包含（M(n+1)） | `node scripts/run-gates.mjs standalone` | skill `verify-standalone.mjs --full`：复制 src/ 到仓库外、`npm install --offline`、`npm run build`；再对复制件的 dist 起纯静态服务逐路由 CLEAN + 零外联 | `standalone/summary.md` + 每路由输出 |
| 外壳字节（策略 A） | `npm run shell:verify:src` / `shell:verify:port` | skill `verify-shell.mjs`：逐文档 patience diff，每个 hunk 必须能由 `scripts/lib/shell-common.mjs` 的变换表回放 | 终端 |
| 零依赖（工具链自检） | `npm run zerodep` | `scripts/` 零依赖、门不 import 生产工具（`tools/`） | — |

`--target port` 让 offline / pixel 门改测 **逐字移植侧**（`port/site` + `--fallback-root mirror`），产物进 `offline-port/`、`pixel-port/`；默认 `--target src` 测 `src/dist`。

前置：`npm run build`（src:shells → src 内 vite build → src/dist；首次先 `npm run assets:restore`）；port 侧 `npm run port:build`。门会自己起两侧 serve.mjs
